...
export class PerformanceModel {
  ...
  public predict(
    ...
    const firstSleepStart = sleepHistory.length > 0
    ...
    while (state.time < targetTime) {
      ...
      const doseNow = caffeineHistory.find(d =>
        d.time.getTime() >= currentTimeMs && d.time.getTime() < nextTime.getTime()
      );
      if (doseNow) {
  ...
  private step(currentState: ModelState, isSleeping: boolean): ModelState {
    ...
    const absorption = C.CAFFEINE_ABSORPTION_RATE * currentState.caffeineInGut * dt;
    const elimination = C.CAFFEINE_ELIMINATION_RATE * currentState.caffeineInPlasma * dt;
  ...
  private calculatePerformance(state: ModelState): number {
    ...
    const caffeineEffect = C.CAFFEINE_MAX_EFFECT * (state.caffeineInPlasma / (C.CAFFEINE_EC50 + state.caffeineInPlasma));
